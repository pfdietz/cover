...
(defpackage :cover-test
  (:use #:cl #:cover-rt))

(in-package :cover-test)
...
(defun do-ck (form)
  ...
  (fmakunbound 'interp-fn)
  (fmakunbound 'compiled-fn)
...
(defmacro tr (&body body)
  `(progn (cover:forget-all)
	  ...
	  (fmakunbound 'eval-exp)
	  (fmakunbound 'eval-non-atomic)
	  (eval
	   '(defun eval-exp (form)
	     "a test function"
...
(deftest hit5 (tr (eval-exp '(list "foo" (+ 1 1) (* 1 2))) (condensed-report))
  ((1 + :REACH DEFUN)
   (2 + :REACH COND)
   (3 - :FIRST-NON-NULL SYMBOLP)))
(deftest hit6 (tr (fmakunbound 'eval-exp)
		  (eval '(defun eval-exp (x) (when (plusp x) (list x))))
		  (condensed-report))
  ((1 - :REACH DEFUN) (1 - :REACH DEFUN)))
(deftest hit7 (tr (fmakunbound 'eval-exp)
		  (eval '(defun eval-exp (x) (when (plusp x) (list x))))
		  (eval-exp 1)
		  (condensed-report))
  ((1 + :REACH DEFUN) (2 + :REACH WHEN) (3 - :NULL PLUSP) (1 - :REACH DEFUN)))
(deftest hit8 (tr (fmakunbound 'eval-exp)
		  (eval
		   '(defun eval-exp (form)
		     "a test function"
		     (cond ((numberp form) form)
		  ...
		  (condensed-report))
  ((1 - :REACH DEFUN) (1 - :REACH DEFUN)))
(deftest hit9 (tr (fmakunbound 'eval-exp)
		  (eval
		   '(defun eval-exp (form)
		     "a test function"
		     (cond ((numberp form) form)
...
(deftest gf1 (tr (fmakunbound 'gf)
		 (cover:forget-all)
		 (defgeneric gf (x y &key))
		 (defmethod gf ((x integer) y &key) 1)
		 (defmethod gf (x (y integer) &key) 2)
		 (defmethod gf (x y &key) 3)
		 (gf 0 nil)
		 (condensed-report))
  ((1 - :REACH DEFMETHOD) (1 - :REACH DEFMETHOD)))

(deftest gf2 (tr (fmakunbound 'gf2)
		 (cover:forget-all)
		 (defgeneric gf2 (x))
		 (defmethod gf2 ((x t)) (if x 1 2))
		 (defmethod gf2 ((x integer)) (if (< x 0) 3 4))
		 (defmethod gf2 ((x vector)) (if (> (length x) 0) (aref x 0) nil))
		 (gf2 t)
		 (gf2 3)
		 (gf2 "a")
		 (condensed-report))
  ((1 + :REACH DEFMETHOD) (2 + :REACH IF) (3 - :NULL X)
   (1 + :REACH DEFMETHOD) (2 + :REACH IF) (3 - :NON-NULL <)
   (1 + :REACH DEFMETHOD) (2 + :REACH IF) (3 - :NULL >)))

(deftest gf3 (tr (fmakunbound 'gf3)
		 (cover:forget-all)
		 (defgeneric gf3 (x &optional y))
		 (defmethod gf3 (x &optional y) (declare (ignore y)) (if (eql x 17) '(a) '(b)))
		 (defmethod gf3 ((x symbol) &optional y) (append (when y (list x)) (call-next-method)))
		 (gf3 17)
		 (gf3 'c t)
		 (condensed-report))
  ((1 + :REACH DEFMETHOD) (2 + :REACH WHEN) (3 - :NULL Y)))


(deftest gf4 (tr (fmakunbound 'gf4)
		 (cover:forget-all)
		 (defgeneric gf4 (x) (:method-combination standard))
		 (defmethod gf4 :around (x) (list 'a (call-next-method)))
		 (defmethod gf4 ((x integer)) (if (evenp x) 'even 'odd))
		 (defmethod gf4 ((x symbol)) 'foo)
		 (list (gf4 0)
		       (condensed-report)))
  ((A EVEN)
   ((1 + :REACH DEFMETHOD) (2 + :REACH IF) (3 - :NULL EVENP)
    (1 - :REACH DEFMETHOD))))

(deftest gf5 (tr (fmakunbound 'gf5)
		 (cover:forget-all)
		 (defgeneric gf5 (x)
		   (:method (x) 1)
		   (:documentation "Boo!")
		   (:method ((x integer)) (if (evenp x) 2 3))
		   (:method ((x real)) (if (zerop x) 4 5))
		   (:method :before ((x symbol)) (error "Just don't"))
		   (:method :around (x) (1+ (call-next-method))))
		 (defmethod gf5 ((x cons)) 6)
		 (list (gf5 11)
		       (condensed-report)))
  (4 ((1 - :REACH :METHOD)
      (1 + :REACH :METHOD)
...
(deftest forget3 (tr (values (cover:forget 1)
				...
				(cond ((numberp form) form)
				      ...
				      (T form))))
			     (condensed-report)))
  t eval-exp eval-exp ((1 - :REACH DEFUN)(1 - :REACH DEFUN)))
...
;  - :FIRST-NON-NULL T  <10>
;- :REACH (DEFUN EVAL-NON-ATOMIC (FORM))  <2>")
(deftest report8 (tr (fmakunbound 'eval-exp)
		     (eval '(defun eval-exp (x) (when (plusp x) (list x))))
		     (let ((cover:*line-limit* 100))
		       (with-output-to-string (s)
			 (cover:report :out s)))) "
...
(cl:defmacro tx (&body body) ;so will work no matter what cover does!
  `(progn (cover:forget-all)
	  (eval '(cover:annotate nil))
	  (mapc #'fmakunbound '(maybe+ maybe- g h))
	  (eval
	   '(defmacro maybe+ (x y)
	     `(if (numberp ,x) (+ ,x ,y))))
	  (eval '(cover:annotate t))
